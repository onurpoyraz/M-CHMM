latent_variable_iffbs_modified_sampler <- function (iter, training = TRUE, select = NULL) {
  update_samples <- function(iter) {
    if (iter == 1) {
      ...
      current$latent_sequences <<- array(as.integer(0), c(params$N, params$L, params$n_chains))
      posterior$latent_sequences <<- array(as.integer(0), c(params$N, params$L, params$n_chains))
    }
    output <- sample_latent_variable_iffbs_modified(current$pi0,
                                                    current$emission,
